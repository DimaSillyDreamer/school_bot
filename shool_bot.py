...
def holidays_today(holidays, pub_holidays, delta_days, day_number = datetime.datetime.now()):
    ...
    for k, v in holidays.items():       
        if v[0] <= (day_number + datetime.timedelta(days=delta_days)) <= v[1]:
            return k
        else:
            return lessons[(day_number + datetime.timedelta(days=delta_days)).weekday()]

def how_much_days_to_the_end():
    holiday_1 = (numpy.busday_count(datetime.date(2022, 10, 29), datetime.date(2022, 11, 6)))
    holidays_2= numpy.busday_count(datetime.date(2022, 12, 31), datetime.date(2023, 1, 8))
    holidays_3 = numpy.busday_count(datetime.date(2023, 4, 8), datetime.date(2023, 4, 16))
